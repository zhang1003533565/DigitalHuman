...
const styles = readFileSync(fileURLToPath(new URL('./LoginPage.css', import.meta.url)), 'utf8')
...
const desktopStyles = styles.slice(0, styles.indexOf('@media (max-width: 960px)'))
assert.match(desktopStyles, /\.auth-field\s*\{[^}]*display:\s*grid;[^}]*gap:\s*18px;/s, 'desktop field wrapper restores the original 8px net error spacing after the -10px message margin')
