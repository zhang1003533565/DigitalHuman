...
import { buildGuideNavigationSearchParams, buildTripPlanSearchParams, hasGuideRecommendations, normalizeGuideChatResult } from '../../node_modules/.tmp/contracts-test/contracts.js'
...
assert.equal(hasGuideRecommendations(resultFromFinalStreamMeta), false)
assert.equal(hasGuideRecommendations(normalizeGuideChatResult({ recommendedRoutes: ['route-3'] })), true)
assert.equal(hasGuideRecommendations(normalizeGuideChatResult({ relatedSpots: ['灵山大佛'] })), true)
