...
const drawer = readFileSync(new URL('ScenicKnowledgePublishDrawer.tsx', import.meta.url), 'utf8')
...
assert.match(api, /previewScenicKnowledgePublication/)
assert.match(api, /publishScenicKnowledge/)
assert.match(api, /withdrawScenicKnowledge/)
...
assert.match(page, /applyStatus === 'applied'/)
assert.match(page, /发布到知识库/)
assert.match(page, /查看知识状态/)
assert.match(page, /状态加载失败/)
assert.match(page, /getScenicKnowledgePrimaryAction/)
assert.match(page, /getScenicKnowledgeStatusPresentation/)
assert.doesNotMatch(page, /游客呈现/)
assert.doesNotMatch(page, /name="audio_enabled"/)
assert.doesNotMatch(page, /name="live_enabled"/)

assert.match(drawer, /撤回知识/)
assert.match(drawer, /markdown/)
assert.match(drawer, /knowledgeName/)
